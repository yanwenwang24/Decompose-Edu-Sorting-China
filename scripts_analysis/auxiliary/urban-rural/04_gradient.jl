...
women_ratio = combine(
    groupby(sample_women, [:urban, :birthy, :edu5]),
...
# Ratio of unmarried to married
men_ratio = combine(
    groupby(sample_men, [:urban, :birthy, :edu5]),
    # Calculate married and unmarried counts
    :marst => (x -> sum(x .== "married")) => :married,
    :marst => (x -> sum(x .!= "married")) => :unmarried
)
...
# Save to outputs
write_parquet("outputs/tables/auxiliary/urban-rural/gradients.parquet", gradients_df)
